...
public class Player : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        {
            if (transform.position.x >= playerPositioner.transform.position.x + 0.05f || transform.position.x <= playerPositioner.transform.position.x - 0.05f)
            {
                velocity += new Vector2((playerPositioner.transform.position.x - transform.position.x) / 20, 0);
            }
